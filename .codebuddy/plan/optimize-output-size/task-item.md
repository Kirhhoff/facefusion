...
- [x] 1. 将帧提取格式从 PNG 改为 JPEG（普通模式 + turbo 模式）
...
- [x] 2. 更新所有帧文件 glob 匹配与扩展名引用（`frame_*.png` → `frame_*.jpg`）
...
- [x] 3. 优化 `reassemble_video` 视频编码：默认使用 libx264 + CRF 模式
...
- [x] 6. 更新 `VIDEO_FACE_SWAP_GUIDE.md` 文档，添加 pixel_boost 参数说明和体积优化指南
